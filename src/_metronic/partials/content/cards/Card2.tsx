...
const Card2: FC<Props> = ({ icon, badgeColor, status, statusColor, title, description, date, budget, progress, users = undefined }) => {
    return (
        <Link to="/crafted/pages/profile/overview" className="card border border-2 border-gray-300 border-hover">
            <div className="card-header border-0 pt-9">
                ...
                <div className="card-toolbar">
                    <span className={`badge badge-light-${badgeColor} fw-bold me-auto px-4 py-3`}>{status}</span>
                </div>
            </div>
            ...
            <div className="card-body p-9">
                ...
                <p className="text-gray-500 fw-semibold fs-5 mt-1 mb-7">{description}</p>

                <div className="d-flex flex-wrap mb-5">
                    <div className="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-7 mb-3">
                        <div className="fs-6 text-gray-800 fw-bold">{date}</div>
                        <div className="fw-semibold text-gray-500">Due Date</div>
                    </div>
                ...
                <div className="h-4px w-100 bg-light mb-5" data-bs-toggle="tooltip" title="This project completed">
                    <div className={`bg-${statusColor} rounded h-4px`} role="progressbar" style={{ width: `${progress}%` }}></div>
                </div>
