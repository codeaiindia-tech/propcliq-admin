        ...
        <div className="card-body d-flex flex-column justify-content-end pe-0">
            ...
            <div className="symbol-group symbol-hover flex-nowrap">
                {items.map((item, index) => (
                    <div className="symbol symbol-35px symbol-circle" data-bs-toggle="tooltip" title={item.name} key={`cw7-item-${index}`}>
                        {item.src && <img alt="Pic" src={item.src} />}
                        {item.state && item.initials && (
                            <span className={clsx('symbol-label fw-bold', 'bg-' + item.state, 'text-inverse-' + item.state)}>{item.initials}</span>
                        )}
                    </div>
                ))}
