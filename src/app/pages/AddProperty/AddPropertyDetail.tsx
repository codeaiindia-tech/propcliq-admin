...
var b = ['', '', 'twenty','thirty','forty','fifty', 'sixty','seventy','eighty','ninety'];
  ...
  function SimpleDialog(props: SimpleDialogProps) {
    ...
    const [amentiesData, setAmentiesData] =  useState(selectedValue);
    ...
    const handleAmenities = (e:any) => {
       ...
       let currentData = amentiesData? amentiesData : [];
      if( e.target.checked === true){
      const d = amenitieseMenuItems.filter(function(item:any) {
          return item.value == e.target.value
      })

        setAmentiesData([...currentData, d[0]])     
      }else {
        currentData = currentData.filter(function(item:any) {
          return item.value !== e.target.value
      })
      setAmentiesData(currentData) 
      }
        ...
        <div className="d-flex flex-wrap" style={{ gap: "16px" }}>
                      <div className='d-flex' style = {{marginTop:'4%'}}>
                        ...
                        <input 
                          ...
                          style={{backgroundColor:'#c4c4c5'}}
                          checked= { (amentiesData?.filter(function(item:any) {
                            return item.value == val.value
                        }))?.length > 0}
                          type='checkbox' key = {index} value={val.value} />
                         <span style={{marginLeft:'5%'}}>{val.icon}</span>  <span style={{marginLeft:'5%'}} className='form-check-label'>{val.label}</span>
                        </label>)})}
                      </div>
                    </div>
    ...
    const Step1: FC<any> = (props: any) => {
      ...
      const [tenantTypeActiveValues, setTenantTypeActiveValues] = useState<any>([]);
    ...
    const getPropertyId: any = params.get('id');
      ...
      const handleSubmitFirstStep = async () => {
        ...
        let data:any = {
          ...
          preferred_tenant_type: tenantTypeActiveValues,
    ...
    setTenantTypeActiveValues(preferred_tenant_type);
    ...
    setOpenParkingMenuActive('');
          ...
          const handleTenantChange = (e:any) => {
            let currentData = tenantTypeActiveValues;
            if( e.target.checked === true){
              setTenantTypeActiveValues([...currentData, e.target.value])
            }else {

              currentData = currentData.filter(function(item:any) {
                return item !== e.target.value
            })
            setTenantTypeActiveValues(currentData) 
            }
              ...
              {propertyTypeActive === "Residential" && (
                <div>
                  ...
                  <div className="add_property-group" style={{ marginTop: "30px" }}>
                    ...
                    <SimpleDialog
                selectedValue={selectedAmenties}
                open={open}
                onClose={handleClose}
                />
...
<div className="d-flex flex-wrap" style={{ gap: "16px" }}>
                      <div className='d-flex'>
                        {selectedAmenties?.map((val:any, index:any) => {
                          return ( 
                          <div style={{padding:'5%', border:'1px solid', borderRadius:'10px', margin:'2%'}}>{val.label}</div>
                          )})}
                          <Button  
